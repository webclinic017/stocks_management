...
class Stock:
    ...
    def update_stock(self):
        ...
        try:
            self.stock_df = yf.download(
                str(self.ticker), start=self.today - timedelta(44), end=self.today
            )

            self.stock_price = round(
                self.stock_df.loc[self.stock_df.index[-1]]["Close"], 2
            )
            ...
            self.prev_close = (
                StockData.objects.filter(ticker=self.ticker).first().prev_close
            )
            self.todays_open = (
                StockData.objects.filter(ticker=self.ticker).first().todays_open
            )
            self.gap_1, self.gap_1_color = gap_1_check(
                self.prev_close, self.todays_open
            )

            self.date = self.stock_df.index[-1]

            self.date = self.stock_df.index[-1]
            self.displayed_date = date_obj_to_date(
                pd.Timestamp("today"), date_format="slash"
            )

            self.week_1, self.week_1_min, self.week_1_max = week_values(
                self.stock_df, 90
            )
            self.week_2, self.week_2_min, self.week_2_max = week_values(
                self.stock_df, 10
            )
            self.week_3, self.week_3_min, self.week_3_max = week_values(
                self.stock_df, 15
            )
            self.week_5, self.week_5_min, self.week_5_max = week_values(
                self.stock_df, 25
            )
            ...
            if np.abs(self.macd_30) > tan_deviation_angle:
                ...
                if (self.trend_30 > 0 and self.macd_30 < 0) or (
                    self.trend_30 < 0 and self.macd_30 > 0
                ):
                    self.macd_30_clash = True
                    self.macd_30_color = "red"
                elif (self.trend_30 < 0 and self.macd_30 < 0) or (
                    self.trend_30 > 0 and self.macd_30 > 0
                ):
                    self.macd_30_clash = False
            ...
            if np.abs(self.macd_14) > tan_deviation_angle:
                ...
                if (self.trend_14 > 0 and self.macd_14 < 0) or (
                    self.trend_14 < 0 and self.macd_14 > 0
                ):
                    self.macd_14_clash = True
                    self.macd_14_color = "red"
                elif (self.trend_14 < 0 and self.macd_14 < 0) or (
                    self.trend_14 > 0 and self.macd_14 > 0
                ):
                    self.macd_14_clash = False
            ...
            if np.abs(self.mfi_30) > tan_deviation_angle:
                ...
                if (self.trend_30 > 0 and self.mfi_30 < 0) or (
                    self.trend_30 < 0 and self.mfi_30 > 0
                ):
                    self.mfi_30_clash = True
                    self.mfi_30_color = "red"
                elif (self.trend_30 < 0 and self.mfi_30 < 0) or (
                    self.trend_30 > 0 and self.mfi_30 > 0
                ):
                    self.mfi_30_clash = False
            ...
            if np.abs(self.mfi_14) > tan_deviation_angle:
                ...
                if (self.trend_14 > 0 and self.mfi_14 < 0) or (
                    self.trend_14 < 0 and self.mfi_14 > 0
                ):
                    self.mfi_14_clash = True
                    self.mfi_14_color = "red"
                elif (self.trend_14 < 0 and self.mfi_14 < 0) or (
                    self.trend_14 > 0 and self.mfi_14 > 0
                ):
                    self.mfi_14_clash = False
            ...
            logger.error(f"Failed getting yfinance stock data for update. Error: {e}")
            return
    ...
    def check_dividend_alert(self):
        stock = StockData.objects.filter(
            ticker=self.ticker
        ).first()  # the "first" is because there might be same ticker two tables.
        dividend_date = stock.dividend_date

        try:
            # Convert from text date to timestamp
            dividend_ts = time.mktime(
                datetime.datetime.strptime(dividend_date, "%d/%m/%Y").timetuple()
            )
            ...
            if (dividend_dt - today_dt).days <= 7 and (
                dividend_dt - today_dt
            ).days >= 0:
    ...
    def check_earnings(self):
        try:
            ...
            earnings_ts = time.mktime(
                datetime.datetime.strptime(earnings_date, "%d/%m/%Y").timetuple()
            )
            today_ts = datetime.datetime.timestamp(self.today)
